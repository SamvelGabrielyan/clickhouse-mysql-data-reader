...
class CHClient(Client):
    ...
    def __init__(self, connection_settings):
        L.info("CHClient() connection_settings={}".format(connection_settings))
    ...
    def verify_connection_settings(self, connection_settings):
        if not connection_settings:
            L.critical("Need CH connection settings")
            sys.exit(0)

        if 'host' not in connection_settings:
            L.critical("Need CH host in connection settings")
            sys.exit(0)
        ...
        if 'port' not in connection_settings:
            L.critical("Need CH port in connection settings")
            sys.exit(0)
